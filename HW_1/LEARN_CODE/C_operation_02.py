...
def gray():
    img_path='E:/Program_File/PYTHON/數位影像處理作業/HW_1/EXAMPLE.jpg'
...
def multicolor():
    img_path='E:/Program_File/PYTHON/數位影像處理作業/HW_1/maya.jpg'
    img_filepath = os.path.splitext(img_path)[0]    # 拆分路徑 & 副檔名，0 為路徑
    img_fileextension = os.path.splitext(img_path)[1]  # 1 為副檔名
    img_filename = os.path.basename(img_filepath)     # 取出檔名不含副檔名
    ...
    # tmp = np.zeros_like(img)                  # 分別存取BGR單通道的圖片
    # tmp[...,0] = img_b
    ...
    # tmp = np.zeros_like(img)                  # 分別存取BGR單通道均化後的圖片
    # tmp[...,0] = img_b_eq
